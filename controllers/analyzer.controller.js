...
export const analyzeCvs = async (req, res) => {
  try {
    ...
    // ------------------ Fetch BatchResume ------------------
    const batchResume = await BatchResume.findOne({ batchId, jobId });
    if (!batchResume || !batchResume.resumes.length) {
      return res.status(404).json({ message: "No resumes found in batch" });
    }
...
`;
        ...
        const payload = {
          ...
          analysis: {
            ...
            scoreBreakdown: { experience, skills, roleFit, education, location, other },
            distributed_scores,
        ...
        const failedPayload = {
          cv: { id: resumeMeta.cvId, filename: resumeMeta.originalName, uploadDate: resumeMeta.uploadedAt },
          analysis: { locked: true, status: "failed", error: err.message, analyzedAt: new Date() },
        };
        res.write(JSON.stringify(failedPayload) + "\n");
        analyzedResumes.push(failedPayload);
